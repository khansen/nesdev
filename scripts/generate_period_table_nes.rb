...
# $clock = 1662607.to_d # PAL
$clock = 1789773.to_d # NTSC

def hz_to_nes_period(hz)
  ($clock / (16.to_d * hz.to_d)).floor
end

octaves_in_hz = [
                                                                                             [55.0,  58.2705,  61.7354],
...
puts "period_table_lo:"
octaves_in_hz.each do |octave|
	puts ".db %s" % (octave.map{|hz| hz_to_nes_period(hz)}).map{|period| "$%02X" % (period & 0xff)}.join(',')
end
puts "period_table_hi:"
octaves_in_hz.each do |octave|
	puts ".db %s" % (octave.map{|hz| hz_to_nes_period(hz)}).map{|period| "$%02X" % (period >> 8)}.join(',')
end
